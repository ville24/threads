...
rssRouter.get('/:id', (request, response, next) => {
  ...
  const getRSS = async (item) => {
    ...
    const datajs = JSON.parse(convert.xml2json(data, {compact: true, spaces: 4}))
      ...
      datajs.rss.channel.item.forEach( (item, index) => {
        if (index < 30 && newsTools.contentFilter(item)) {
          const newItem = {
            ...
            title: item.title._text ? purify.purifyTitle(item.title._text) : purify.purifyTitle(item.title._cdata),
          ...
          newItem.imgurl = purify.purifyURL(newsTools.findFixImage({
            ...
            description: item.description._text ? item.description._text : item.description._cdata
          }))
    ...
    const conf = new Conf( { _id: request.params.id, type: 'newsSource' } )
