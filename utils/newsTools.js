const descriptionCut = (text) => {
  ...
  matches
    ...
    .map((match) => {
      ...
      !found && match.index < 200 && (text = text.substring(
        0,
        match.index + 2
      )) && (found = true)

    })
  !found && (text = text.substring(
    0,
    200
  ) + '...')
...
const findFixImage = (item) => {
  ...
  img && (img = img.replace(
    /images.cdn.yle.fi\/image\/upload\/(.*?)\/(.*?)\.jpg/gm,
    'images.cdn.yle.fi/image/upload//w_800,q_auto/$2.jpg'
  ))
...
const contentFilter = (item) => {
  ...
  return item['dc:creator'] && item['dc:creator']._cdata.match(/toni jalovaara/i)
    ? false
    : true
